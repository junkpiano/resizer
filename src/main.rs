...
fn encode(img: &DynamicImage, fmt: OutFormat, quality: u8) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
    {
        ...
        match fmt {
            ...
            OutFormat::Webp => {
                // WebP supports alpha but we convert to RGB for consistency with JPEG
                let rgb = img.to_rgb8();
                let (w, h) = rgb.dimensions();
            ...
            OutFormat::Png => {
                let level = (quality as u32 * 9 / 100).min(9) as u8;
...
fn main() -> Result<()> {
    ...
    // JPEG and WebP convert to RGB (dropping alpha), PNG preserves alpha if present.
    // Transparent pixels in JPEG/WebP become black.
